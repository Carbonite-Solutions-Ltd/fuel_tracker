...
class FuelBalance(Document):
    def on_submit(self):
        # Only call create_fuel_entry if fuel_used_type is not empty
        if self.fuel_used_type:
            self.create_fuel_entry()

    def on_update_after_submit(self):
        # Similarly, only call create_fuel_entry on updates if fuel_used_type is not empty
        if self.fuel_used_type:
            self.create_fuel_entry()
    ...
    def create_fuel_entry(self):
        ...
        # Determine which document to fetch based on the fuel_used_type
        if self.fuel_used_type == "Supplied" and hasattr(self, 'fuel_supplied_id') and self.fuel_supplied_id:
            fuel_supplied_doc = frappe.get_doc("Fuel Supplied", self.fuel_supplied_id)
        elif self.fuel_used_type == "Dispensed" and hasattr(self, 'fuel_used_id') and self.fuel_used_id:
            fuel_utilization_doc = frappe.get_doc("Fuel Used", self.fuel_used_id)
        ...
        new_fuel_entry = frappe.get_doc({
            ...
            "site": self.site,
            "utilization_type": self.fuel_used_type,
    ...
    def get_previous_balance(self, fuel_supplied_doc, fuel_utilization_doc):
        # Calculate the previous balance based on the document type and fuel_used_type
        if self.fuel_used_type == "Supplied" and fuel_supplied_doc:
            return self.balance - fuel_supplied_doc.fuel_supplied
        elif self.fuel_used_type == "Dispensed" and fuel_utilization_doc:
